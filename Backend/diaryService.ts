...
export async function listEntries(): Promise<EntriesSaved[]> {
  ...
  return entries.rows.map((row: any) => ({ id: row.id, title: row.title, entry: row.entry, userId: row.user_id, categoryId: row.category_id, date: row.date, edited: row.edited, link: row.link, categoryName: row.name, colourCode: row.colour_code }))
}
...
export async function addEntry(entries: Entries): Promise<EntriesSaved> {
  ...
  return { id: entry.rows[0].id, title: entry.rows[0].title, entry: entry.rows[0].entry, userId: entry.rows[0].user_id, categoryId: entry.rows[0].category_id, link: entry.rows[0].link, date: entry.rows[0].date, edited: entry.rows[0].edited };
} // do I need to specifically state the userId I want to insert?
