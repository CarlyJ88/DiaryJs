...
export async function selectEntryById(id: number, incomingUserId: string): Promise<EntriesSaved[]> {
  const entries = await executeQuery('SELECT e.*, c.name, c.colour_code FROM entries e INNER JOIN categories c ON e.category_id = c.id WHERE e.id = $1 AND e.user_id = $2', [id, incomingUserId]);
  return entries.rows.map((row: any) => ({ id: row.id, title: row.title, entry: row.entry, incomingUserId: row.user_id, categoryId: row.category_id, date: row.date, edited: row.edited, link: row.link, categoryName: row.name, colourCode: row.colour_code }))
}
